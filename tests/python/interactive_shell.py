...
Usage: interactive_shell.py [--interrupt-on TEXT] PROMPT SECONDS SHELL [ARG...] < lines
...
With --interrupt-on, each time TEXT appears in the output again the driver
types Ctrl-C, which is how a person stops a program that is taking too long.
...
def main():
    ...
    interrupt_on = None
    if arguments[0] == "--interrupt-on":
        interrupt_on = arguments[1].encode()
        arguments = arguments[2:]
    prompt = arguments[0].encode()
    deadline = time.monotonic() + float(arguments[1])
    command = arguments[2:]
    ...
    while status is None:
        ...
        if interrupt_on is not None:
            count = output.count(interrupt_on)
            if count > interrupts_sent:
                interrupts_sent = count
                os.write(fd, b"\x03")

...
- **Installing the `attention` command changes what writers do.** Once it is built, the plugin exports `WEZTERM_ATTENTION_ROOT` to new panes, and the Pi extension writes v2 records through the command instead of v1 flat markers. Those writes need a launch claim. On macOS Pi claims its own pane (see Added); elsewhere Pi started in a pane without a shell claim is refused and shows nothing.
...
- **The default state directory follows `XDG_STATE_HOME`.** The order is `WEZTERM_ATTENTION_DIR`, then `$XDG_STATE_HOME/wezterm-attention` when `XDG_STATE_HOME` is set, non-empty and absolute, then `~/.local/state/wezterm-attention`. 0.6 always used the last. If `XDG_STATE_HOME` is set where WezTerm starts, the directory moves; set `dir` to keep the old one. A `dir` option or `WEZTERM_ATTENTION_DIR` that is not UTF-8 is ignored with a warning by the plugin, and by the Pi extension without the `attention` command configured.
...
- `result.timing_ms` on `inspect` as on `bindings`, and `result.diagnostic_count` / `total_diagnostic_count` on `tabs`. Diagnostics from `bindings`, `inspect`, `tabs` and `doctor` name the record path or pane they are about; `sweep` names the claim and current-binding pointer it could not read.
...
- The plugin and the Pi extension ignore a `WEZTERM_ATTENTION_DIR` or `XDG_STATE_HOME` that is not UTF-8, and say so in their logs, as the attention command refuses it where it decides the state root, instead of each reading a directory the command never writes. The plugin's `dir` option is held to the same rule.

...
- `result.timing_ms` on `inspect` as on `bindings`, and `result.diagnostic_count` / `total_diagnostic_count` on `tabs`. Record diagnostics from `bindings`, `inspect`, `tabs` and `doctor` name the record path they are about; `sweep` names the claim and current-binding pointer it could not read.

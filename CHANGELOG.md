...
- **The Pi extension writes only through the `attention` command.** Where `WEZTERM_ATTENTION_ROOT` is unset in a WezTerm pane (the command is not built, or the pane was opened before it was), Pi records nothing and shows one warning saying so, and outside WezTerm it records nothing and says nothing; `PI_WEZTERM_ATTENTION_TTL_MS` is gone with the flat writer. The plugin exports `WEZTERM_ATTENTION_ROOT` to new panes once the command is built. Pi's writes need a launch claim. On macOS Pi claims its own pane (see Added); elsewhere Pi started in a pane without a shell claim is refused and shows nothing.
...
- The plugin writes no pane record itself. `Alt+B` and the acknowledgement of a focused pane's `stop` or `notify` run the `attention` command, under the locks every writer of those records takes. An acknowledgement is written only while the activity the user was shown is still the pane's, so a newer one is never dismissed unseen. WezTerm waits on the command, so it waits at most 50 ms for each lock, where a hook waits 2 s. An acknowledgement that failed in a way that can pass (a held lock, a command that did not start or did not answer) is tried again after 2, 5, 10, then every 30 seconds; a refusal or an answer stands for that activity, and each kind of failure is logged once per pane. A command that exits without answering is logged as one that may predate the plugin, with the `scripts/install-cli.sh` to run. `Alt+B` withdraws your flag whatever launch the pane's claim names, and sets it only where the claim names the launch the pane published.

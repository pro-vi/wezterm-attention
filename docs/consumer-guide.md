...
**There is no freshness contract.** The file is written when a window's composed list changes or first gains source identity. Attaching that identity preserves the content timestamp, so `published_at_ms` is when the bar last drew something different, not when anything checked. Nothing refreshes it while the bar is idle. When a window closes, the next poll of another window in the same WezTerm process removes its file; a window whose whole WezTerm process has exited leaves its last file behind, and `attention sweep` collects that file once every pane it names is verified absent, or once it names no tab at all. A file naming a pane by a bare decimal id is kept, because such an id names no realm to ask. Use this to describe tabs and to resolve an ordinal — "the second `bootstrap` tab" — where a wrong answer is visible to the person who asked. To act on a tab, ask the GUI: inside WezTerm, `mux_window:tabs_with_info()` returns the same order live.

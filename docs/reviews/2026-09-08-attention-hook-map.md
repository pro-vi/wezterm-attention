...
Provider definitions: [Claude Code hooks](https://code.claude.com/docs/en/hooks), [Codex hooks](https://learn.chatgpt.com/docs/hooks), [Codex release notes](https://learn.chatgpt.com/docs/changelog), and, for Pi, `docs/extensions.md` and `dist/core/extensions/types.d.ts` in the installed `@earendil-works/pi-coding-agent` 0.84.4 package.

Attention behavior at the time of writing: provider normalization in [`src/providers.rs`](../../src/providers.rs), Pi native subscriptions and shutdown policy in [`pi/index.ts`](../../pi/index.ts), and Lua acknowledgement filtering in [`plugin/reader.lua`](../../plugin/reader.lua). Those files have changed since; read them for current behavior, not this table.

The complete provider name sets were checked against this table when it was written; that catalog is not kept in this repository. The check covered names, not runtime semantics. No runtime files or live configuration changed in this work.

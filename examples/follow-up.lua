...
-- The callback updates local presentation only. Initial/cached views never
-- replay a sound, clipboard operation, input or other application action.
function M.for_windows()
  local policies, appearances, views = {}, {}, {}
  local function key(window_id, source)
    local target = source.target or {}
    return table.concat({tostring(window_id),source.address.realm_id,source.address.incarnation_id,
      source.address.pane_id,source.launch_id,target.kind or "",target.binding_id or ""}, "\0")
  end
  return {
    on_view_change = function(change)
      local source=change.scope or change.previous_scope
      local id=key(change.window_id,source)
      if change.kind=="scope_lost" then policies[id],appearances[id],views[id]=nil,nil,nil;return end
      if change.kind=="initial" or not policies[id] then policies[id]=M.new() end
      appearances[id]=policies[id].appearance(change.view)
      views[id]=change.view
    end,
    appearance = function(window_id, source) return appearances[key(window_id,source)] or "unknown" end,
    dismiss = function(window_id, source)
      local id=key(window_id,source)
      local policy=policies[id]
      if policy then policy.dismiss();appearances[id]=policy.appearance(views[id]) end
    end,
  }
end

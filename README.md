...
> **Note:** If you change the `dir` option in `apply_to_config`, update the `markerDir` path in your hooks to match.
...
**Markers not showing?**
- Check the directory exists: `ls ~/.claude/wezterm-attention/`
- Verify `WEZTERM_PANE` is set: `echo $WEZTERM_PANE` (should print a number inside WezTerm)
- Check file contents: `cat ~/.claude/wezterm-attention/$WEZTERM_PANE` (should be valid JSON)
- Confirm `status_update_interval` is set (default 1000ms; markers update on this interval)
